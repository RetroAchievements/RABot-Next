...
const tpollCommand: Command = {
  ...
  async execute(message, args, client) {
    ...
    if (firstSpaceIndex === -1) {
      await message.reply("Usage: `!tpoll <seconds> 'Question?' 'Option 1' 'Option 2' ... 'Option N'`");
      return;
    }
    ...
    if (isNaN(seconds) || seconds < 0 || seconds > 604800) { // Max 1 week.
    ...
    const filter = (reaction: any, user: any) => {
      ...
      if (optionIndex !== -1) {
        ...
        const currentVotes = pollResults.get(reaction.emoji.name) || 0;
        pollResults.set(reaction.emoji.name, currentVotes + 1);
      }
    ...
    collector.on('end', async () => {
      try {
        ...
        if (pollResults.size === 0) {
          ...
          sortedResults.forEach(([emoji, count]) => {
